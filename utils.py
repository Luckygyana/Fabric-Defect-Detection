...
def plot_sample(
    ...
    if blur:
        ...
        plt.imshow(
            (segmentation / segmentation.max() * 255).astype(np.uint8), cmap="jet"
        )

    out_prefix = "{:.3f}_".format(decision) if decision is not None else ""

    plt.savefig(
        f"{save_dir}/{out_prefix}result_{image_name}.jpg", bbox_inches="tight", dpi=300
    )
    plt.close()

    if plot_seg:
        jet_seg = cv2.applyColorMap(
            (segmentation * 255).astype(np.uint8), cv2.COLORMAP_JET
        )
