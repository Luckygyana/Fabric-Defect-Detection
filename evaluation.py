...
def get_performance_eval(
    P, Y, names, data_dir, output_dir, folds, prefix="", thresholds_tups=None, save=True
):
    metrics = {}
    ...
    for thresh, name, dir in zip(
        [best_threshold, fn0_threshold, 0.5],
        ["best", "fn0", "50_perc"],
        ["best", "fn0", "50_perc"],
    ):
        FN, FP, TN, TP = get_and_copy_falses(
            P,
            Y,
            thresh,
            data_dir,
    ...
    if save:
        for thr, d in metrics["thresholds"].items():
            print(
                f'THRESHOLD {prefix} {thr:>15} => VALUE={d["value"]:.4f}, FP={d["FP"]} FN={d["FN"]}, AP={AP}'
            )

    return metrics


def get_and_copy_falses(
    P, Y, best_threshold, data_dir, folds, names, output_dir, prefix, save=True
):
    ...
    if save:
        ...
        FN_names = [
            (folds[i], names[i] + ".jpg") for i in range(0, len(names)) if FN[i]
        ]
        FP_names = [
            (folds[i], names[i] + ".jpg") for i in range(0, len(names)) if FP[i]
        ]
    ...
    for fold, n in names:
        ...
        if len(f_name) > 0:
            ...
            dst_file = os.path.join(
                output_dir, f'{"FN" if is_FN else "FP"}_{acc}_{f_name[6:]}'
            )
...
def evaluate_fold(results_folder, t_folds, t_gt, t_img_names, t_preds):
    m_test = evaluate_decision(
        ...
        t_preds,
        prefix="",
        output_dir=os.path.join(results_folder),
        save=False,
    )
    ...
    cls_acc = (best["TP"] + best["TN"]) / (
        best["TP"] + best["TN"] + best["FP"] + best["FN"]
    )
    cls_acc_50 = (t50["TP"] + t50["TN"]) / (
        t50["TP"] + t50["TN"] + t50["FP"] + t50["FN"]
    )
...
def read_predictions(fold, prefix, run_dir):
    ...
    if fold is not None:
        fold_path = os.path.join(
            run_dir, "FOLD_{}".format(fold), prefix + "results.csv"
        )
        decisions, folds, ground_truth, img_names, predictions = read_directory(
            decisions, fold, fold_path, folds, ground_truth, img_names, predictions
        )
    else:
        results_path = os.path.join(run_dir, prefix + "results.csv")
        decisions, folds, ground_truth, img_names, predictions = read_directory(
            decisions, 0, results_path, folds, ground_truth, img_names, predictions
        )
    img_names = list(map(str, img_names))
    predictions, decisions, ground_truth, img_names, folds = list(
        map(np.array, [predictions, decisions, ground_truth, img_names, folds])
    )
...
def read_directory(
    decisions, f, fold_path, folds, ground_truth, img_names, predictions
):
