...
def read_results(
    results_path, dataset, folds=None, dagm_join=False, sortkey=itemgetter(0)
):
    results = []
    results_columns = [
        "RUN_NAME",
        ...
        try:
            ...
            for f in folds:
                (
                    ...
                    t_img_names,
                    t_preds,
                ) = evaluation.read_predictions(f, "", run_path)
                fold_eval_res = evaluation.evaluate_fold(
                    run_path, t_folds, t_gt, t_img_names, t_preds
                )
                if len(folds) == 1:
            ...
            results.append(
                [
                    ...
                    cfg.TRAIN_NUM,
                    cfg.NUM_SEGMENTED,
                    cfg.WEIGHTED_SEG_LOSS,
                    cfg.WEIGHTED_SEG_LOSS_P,
                    cfg.WEIGHTED_SEG_LOSS_MAX,
                    cfg.FREQUENCY_SAMPLING,
                    cfg.DYN_BALANCED_LOSS,
                    cfg.DELTA_CLS_LOSS,
                    cfg.EPOCHS,
                    cfg.LEARNING_RATE,
                    f"{auc:.5f}",
...
def main():
    ...
    results = read_results(
        results_folder, dataset, folds_dict[dataset], dagm_join, sortkey=itemgetter(0)
    )
    results.to_csv(
        os.path.join(
            results_folder,
            f'{dataset}_summary{f"_joined" if dataset == "DAGM" and dagm_join else ""}.csv',
        ),
        index=False,
    )
